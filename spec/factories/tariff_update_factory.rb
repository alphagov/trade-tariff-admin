FactoryGirl.define do
  factory :tariff_update do
    ...
    trait :with_exception do
      ...
      exception_queries "(ActiveRecord::Mysql2::Database) BEGIN \n(ActiveRecord::Mysql2::Database) SELECT * FROM `measures` LIMIT 1 \n(ActiveRecord::Mysql2::Database) ROLLBACK \n(ActiveRecord::Mysql2::Database) UPDATE `tariff_updates` SET `state` = 'F', `updated_at` = '2014-07-22 16:16:15' WHERE ((`tariff_updates`.`update_type` IN ('TariffSynchronizer::TaricUpdate')) AND (`filename` = '2014-07-22_TGB14203.xml')) LIMIT 1 "
